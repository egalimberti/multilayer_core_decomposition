...
from core_decomposition.subroutines.commons import filter_inner_most_cores
...
if __name__ == '__main__':
    # create a parser
    parser = argparse.ArgumentParser(description='Filter Inner-most Cores')
    ...
    # create the new file for inner-most cores only
    distinct_cores_file = open(dirname(dirname(getcwd())) + '/output/' + args.cd + '_i.txt', 'w')

    # print the name of the input graph
    print_dataset_name(args.cd.split('_')[0])
    print '--------- Inner-most Cores --------'
    ...
    # filter inner most cores
    filter_inner_most_cores(cores)
